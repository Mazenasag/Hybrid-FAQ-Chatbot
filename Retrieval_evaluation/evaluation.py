...
# --------------------- DATA LOADING --------------------- #
def load_data():
    with open("Retrieval_evaluation/final.json") as f:
        faq_data = json.load(f)
    with open("Retrieval_evaluation\evdata_copy.json") as f:
        test_queries = json.load(f)
    return faq_data, test_queries

# --------------------- VECTOR STORE --------------------- #
def create_vector_store(embeddings, faq_data):
    # Build documents using only the question text
    documents = [
        Document(
            page_content=item["question"],  # only question
            metadata={
                "answer": item["answer"],
                "original_question": item["question"],
                "index": idx  # keep the index for reference
            }
        )
...
# def create_vector_store(embeddings, faq_data):
#     """
#     Create a FAISS vector store from FAQ data including keywords.
    
#     Parameters:
#         embeddings : Embeddings object
#         faq_data   : List of dicts, each with 'question', 'answer', and optionally 'keywords'
...
def hybrid(query, vector_store, bm25_index, faq_data):
    # Semantic scores (using TOP_K=3 for fair comparison)
...
def evaluate(method_fn, test_queries, *args):
    ...
    for sample in test_queries:
        ...
        if sample["expected"] == retrieved[0]:
            metrics['top1_acc'] += 1
        if sample["expected"] in retrieved:
            metrics['recall@3'] += 1
...
def log_results_to_json(results, hybrid_details, config, file_path="lognokeywr.json"):
    log_entry = {
        "TOP_K": config["TOP_K"],
...
if __name__ == "__main__":
    ...
    # Add total_queries to hybrid metrics
