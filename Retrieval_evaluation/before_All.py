...
# --------------------- DATA LOADING --------------------- #
def load_data():
    faq_path = r"Retrieval_evaluation/final_80.json"
    test_path = r"Retrieval_evaluation/evdata_copy.json"
...
#     faq_path = os.path.join(script_dir, "final.json")
#     test_path = os.path.join(script_dir, "evdata.json")
    
#     # Try alternative paths if the files aren't found
#     if not os.path.exists(faq_path):
#         # Try one level up
#         parent_dir = os.path.dirname(script_dir)
#         faq_path = os.path.join(parent_dir, "final.json")
#         test_path = os.path.join(parent_dir, "evdata.json")
        
#         if not os.path.exists(faq_path):
#             # Try in the Retrieval_evaluation directory
#             faq_path = os.path.join(parent_dir, "Retrieval_evaluation", "final.json")
#             test_path = os.path.join(parent_dir, "Retrieval_evaluation", "evdata.json")
    
#     print(f"Looking for FAQ data at: {faq_path}")
...
#     print(f"Successfully loaded {len(faq_data)} FAQ items and {len(test_queries)} test queries")
#     return faq_data, test_queries
...
#     tokenized_questions = []
#     for item in faq_data:
#         text = item['question']  # only question, no keywords
#         processed = text.lower().translate(str.maketrans('', '', string.punctuation))
#         tokenized_questions.append(processed.split())
#     return BM25Okapi(tokenized_questions)
...
#         sem_score = sem_scores.get(item["question"], 0)
#         norm_bm25 = (bm25_scores[idx] - min(bm25_scores)) / (max(bm25_scores) - min(bm25_scores) + 1e-9)
#         final_score = HYBRID_WEIGHTS[0] * sem_score + HYBRID_WEIGHTS[1] * norm_bm25
#         combined_scores.append((final_score, idx))
...
#         # Handle multiple expected questions
#         expected_list = sample["expected"] if isinstance(sample["expected"], list) else [sample["expected"]]
...
#         # Recall@3
#         if any(r in retrieved[:3] for r in expected_list):  # Fixed slice index
#             metrics['recall@3'] += 1
#     total = len(test_queries)
#     metrics['top1_acc'] = metrics['top1_acc'] / total * 100
#     metrics['recall@3'] = metrics['recall@3'] / total * 100
