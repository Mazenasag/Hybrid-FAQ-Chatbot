...
def evaluate(method_fn, test_queries, *args):
    metrics = {'top1_acc': 0, 'recall@3': 0, 'mrr': 0, 'ndcg@5': 0, 'response_times': []}
    for sample in test_queries:
        ...
        for rank, doc in enumerate(retrieved[:5], start=1):
            rel = 1 if doc in expected_list else 0
            dcg += rel / math.log2(rank + 1)
        ideal_rel = [1] * min(len(expected_list), 5)
        idcg = sum(rel / math.log2(idx + 2) for idx, rel in enumerate(ideal_rel))
    ...
    metrics['ndcg@5'] = metrics['ndcg@5'] / total
...
if __name__ == "__main__":
    ...
    # Print results
    print("\nEvaluation Results:")
    print("| Method          | Top-1 Acc | Recall@3 |   MRR   | nDCG@5 | Avg Time (ms) |")
    print("|-----------------|-----------|----------|---------|--------|---------------|")
    for name, metrics in results.items():
        print(f"| {name:<15} | {metrics['top1_acc']:>8.2f}% | {metrics['recall@3']:>8.2f}% |"
              f" {metrics['mrr']:.3f} | {metrics['ndcg@5']:.3f} | {metrics['avg_time']:>13.2f} |")
